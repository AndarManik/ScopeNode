import {
  MINKOWSKI_CIRCLE_DEG,
  OBSTACLE_AREA,
  OBSTACLE_MIN_ANGLE_DEG,
} from "./global.js";

export function transformPoints([tx, ty], theta, points) {
...
export function anglesFromAlpha(alpha, minDeg = OBSTACLE_MIN_ANGLE_DEG) {
...
export function spawnTriangleCentered(
  alpha,
  area = OBSTACLE_AREA,
  minDeg = OBSTACLE_MIN_ANGLE_DEG
) {
...
export function minkowskiSum(tri, r, angleStepDeg = MINKOWSKI_CIRCLE_DEG) {
  angleStepDeg = Math.min(120, Math.max(0.1, angleStepDeg));
  const dTheta = (angleStepDeg * Math.PI) / 180;
  const rEff = r / Math.cos(dTheta / 2);
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (!(nCurr[0] === 0 && nCurr[1] === 0)) {
      const s1 = add(viNext, [nCurr[0] * r, nCurr[1] * r]);
